...
const APPLE_EASE = [0.25, 0.1, 0.25, 1.0] as any;
...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show sticky navbar after user scrolls past 600px (roughly 2 sections)
  ...
  const navLinks = [
    ...
    { name: 'For Organizations', href: '#orgs' },
            ...
            <div className="container h-full flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 bg-ink rounded-sm flex items-center justify-center">
                  <div className="w-2 h-2 border-2 border-white rounded-full opacity-50"></div>
                </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <button className="btn-primary py-1 px-4 text-[12px]">Get Protocol</button>
              </div>
            </div>
          </motion.nav>
        )}
...
// @ts-ignore
import coverImg from './cover-image.jpg';
// @ts-ignore
  ...
  return (
    <section id="overview" className="relative md:h-screen flex flex-col justify-end overflow-hidden pt-[56px] pb-12 md:pb-24 bg-white">
      ...
      <div className="relative md:absolute inset-0 z-0 h-[240px] md:h-full">
        ...
        {/* Apple-style overlays: gradual blurry filter and white-to-transparent gradient */}
        <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button className="flex items-center gap-2 text-accent font-bold text-[18px] hover:underline">
...
const Highlights = () => {
  ...
  const items = [
    {
      metric: '30 seconds',
      desc: 'From signal to support in under a minute.',
      icon: Clock,
      modalTitle: 'Speed saves lives.',
      longDesc: 'The Butterfly Protocol is built on the scientific principle of implementation intentions. By reducing the time between a signal and a response, we eliminate the "bystander effect" and provide immediate stability before a situation escalates.'
    },
    {
      metric: '£4.70 return',
      desc: 'Average ROI in workplace mental health programs.',
      icon: BarChart3,
      footnote: '1',
      modalTitle: 'The Business Case',
      longDesc: 'Mental health turnover and attrition costs organizations billions annually. Research by Deloitte UK indicates that for every £1 spent on mental health support, organizations see a £4.70 return through reduced absenteeism and higher engagement.'
    },
    {
      metric: 'Upstream adoption',
      desc: 'Designed for the 97% who never call EAP.',
      icon: Users,
      modalTitle: 'Broad Coverage',
      longDesc: 'Most workplace mental health programs (EAPs) have a utilization rate of less than 3%. The Butterfly Protocol is designed for everyone else — the 97% who are struggling but haven\'t yet reached a clinical crisis point.'
    },
    {
      metric: 'Governed standard',
      desc: 'Versioned. Auditable. Improvable.',
      icon: Shield,
      modalTitle: 'Safety Registry',
      longDesc: 'Unlike "wellbeing workshops," the Butterfly Protocol is an operational safety standard. It is version-controlled, auditable, and designed to be integrated into existing risk management frameworks (ISO 45003).'
    },
    {
      metric: 'Privacy-first',
      desc: 'No PII. No diagnosis. No surveillance.',
      icon: Lock,
      modalTitle: 'Absolute Privacy',
      longDesc: 'By strictly separating "recognition and routing" from clinical "therapy," we ensure that no personally identifiable information (PII) is ever recorded in HR systems. This protects the employee and reduces organization liability.'
    },
  ];
  ...
  return (
    <section className="section !px-0 bg-bg-muted/50 overflow-hidden pb-32">
      <div className="container mb-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <FadeIn>
            ...
            <h2 className="mb-0 text-transparent bg-clip-text bg-gradient-to-r from-ink to-accent">Get the highlights.</h2>
          </FadeIn>

          <a href="#" className="flex self-start items-center gap-1 text-accent font-bold hover:underline mb-2">
            Read the white paper <ChevronRight size={18} />
          </a>
        </div>
      </div>

      {/* Outer wrapper spans full viewport for edge-to-edge scrolling */}
      <div className="relative w-full overflow-visible">
        <div
          ref={scrollRef}
          className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-12 cursor-grab active:cursor-grabbing w-full"
        >
          {/* Left Spacer to align the first card with the grid initially. Snaps to start to allow restoring the grid alignment. */}
          <div
            className="shrink-0 snap-start pointer-events-none"
            style={{ width: 'calc(max(24px, calc((100vw - 1536px) / 2 + 24px)) - 24px)' }}
          />
          {items.map((item, i) => (
            <FadeIn
              key={i}
            ...
            >
              <div
                className="card h-full w-[280px] md:w-[360px] min-h-[440px] flex flex-col justify-between bg-white hover:shadow-xl transition-all border-none p-10 cursor-pointer group"
              ...
              >
                <div className="mb-auto">
                  ...
                  <div className="font-bold text-[28px] md:text-[32px] mb-4 leading-tight">
                    {item.metric}
                  </div>
          ...
          {/* Right Spacer to ensure the last card aligns with the grid on the right. */}
          <div
            className="shrink-0 snap-start pointer-events-none"
            style={{ width: 'calc(max(24px, calc((100vw - 1536px) / 2 + 24px)) - 24px)' }}
          />
        ...
        <div className="container">
          ...
          <div className="flex justify-start md:justify-end gap-3 mt-4">
            <button
              onClick={() => scroll('left')}
              className="w-12 h-12 rounded-full border border-hair bg-white flex items-center justify-center hover:bg-bg-muted transition-colors shadow-sm"
            >
              <ArrowLeft size={20} />
            </button>
            <button
      ...
      <AnimatePresence>
        {activeItem && (
          <HighlightModal item={activeItem} onClose={() => setActiveItem(null)} />
        )}
      </AnimatePresence>
    </section>
  );
    ...
    const chart = window.AmCharts.makeChart("chartdiv", {
      ...
      graphs: [
        ...
        },
        {
          ...
          balloonText: "<div style='margin-bottom:30px;text-shadow: 2px 2px rgba(0, 0, 0, 0.1); font-weight:600;font-size:24px; color:#00b18d'>[[value]]</div>"
        }
      ],
      chartCursor: {
        cursorAlpha: 1,
...
const Category = () => (
  <section className="relative w-full overflow-hidden bg-black h-[50rem] md:h-auto">
    {/* Background Image - drives the exact height of the section to prevent cropping */}
    ...
    <div className="absolute inset-0 z-10 flex flex-col justify-between pt-12 md:pt-16 pb-8 md:pb-24 px-6 md:px-0">
      <div className="container mx-auto">
        <FadeIn className="text-center md:mt-12">
          {/* Headline - mimicking "Superior sound down to a science." */}
          ...
          <div className="card bg-bg-muted border-none p-6">
            ...
            <ul className="space-y-3">
              <li className="flex gap-3">
                ...
                <span>Immediate danger → 911 first</span>
              </li>
              <li className="flex gap-3">
                <span className="text-ink font-bold">•</span>
                <span>Safe and accessible → Proceed to Step 2</span>
              </li>
            </ul>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-hair">
                  <th className="py-4 font-bold text-caption text-[12px] uppercase">Response</th>
                  <th className="py-4 font-bold text-caption text-[12px] uppercase">Your Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-hair">
                <tr>
                  <td className="py-4 font-medium italic pr-4">"I just need a moment"</td>
                  <td className="py-4">"Take the time you need. I'll check back in 30 minutes."</td>
                </tr>
                <tr>
                  <td className="py-4 font-medium italic pr-4">"I'd like to talk to someone"</td>
                  <td className="py-4">Open the Resource Map together. Walk them to the door.</td>
                </tr>
                <tr>
                  <td className="py-4 font-medium italic pr-4">Signs of immediate risk</td>
                  <td className="py-4">"Let's call 988 together right now." Stay with them.</td>
                </tr>
                <tr>
                  <td className="py-4 font-medium italic pr-4">Refuses all support</td>
                  <td className="py-4">Document. Escalate to your HR/clinical lead today.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card border-accent/20 bg-accent-light/20 p-8">
            <h4 className="font-bold text-ink mb-4 flex items-center gap-2">
              <Shield size={20} className="text-accent" /> NON-NEGOTIABLE DATA RULES:
            </h4>
            <ul className="space-y-3 text-[15px]">
              <li className="flex gap-3">
                <span className="text-accent font-bold">/</span>
                <span>Logs stored in SEPARATE system from HR performance records — never in HRIS</span>
              </li>
              <li className="flex gap-3">
                <span className="text-accent font-bold">/</span>
                <span>Automatic 90-day retention limit — then purged</span>
              </li>
              <li className="flex gap-3">
                <span className="text-accent font-bold">/</span>
                <span>Logs may NOT be referenced in performance reviews, disciplinary actions, PIPs, or termination proceedings</span>
              </li>
              <li className="flex gap-3">
                <span className="text-accent font-bold">/</span>
                <span>Model data governance policy included in deployment kit for your GC</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="flex border-b border-hair overflow-x-auto no-scrollbar">
            {steps.map((step, i) => (
              <button
                ...
                className={cn(
                  "px-6 py-4 text-[14px] font-bold tracking-wider transition-all border-b-2 whitespace-nowrap",
                  activeTab === i ? "border-accent text-accent" : "border-transparent text-caption hover:text-ink"
                )}
              >
                {step.title}
              </button>
...
const Comparison = () => (
  <section className="section bg-bg-muted/30">
    <div className="container">
      <FadeIn>
        <h2 className="mb-12">What it is. What it isn't.</h2>
      </FadeIn>
      <div className="grid md:grid-cols-2 gap-px bg-hair border border-hair rounded-3xl overflow-hidden shadow-sm">
        <div className="bg-white p-8 md:p-12">
          ...
          <ul className="space-y-4">
            {[
              "Repeatable 3-step script",
              "Routes to 988, EAP, counselor",
              "Auditable record without PII/PHI",
              "10-minute training",
              "Strengthens EPLI defense posture"
            ].map(item => (
              <li key={item} className="flex gap-3 text-ink">
                <Check size={18} className="text-ok mt-1 shrink-0" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="bg-white p-8 md:p-12">
          ...
          <ul className="space-y-4">
            {[
              ...
              "Require clinical staff to administer",
              "Collect personal health information",
              "Create new clinical liability"
            ].map(item => (
              <li key={item} className="flex gap-3 text-muted">
                <X size={18} className="text-muted mt-1 shrink-0 opacity-40" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
    ...
    <section id="evidence" className="section">
      <div className="container">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-2 border-ink">
                <th className="py-6 font-bold text-ink text-[16px] pr-8">Claim</th>
                <th className="py-6 font-bold text-ink text-[16px]">Evidence</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-hair">
              {data.map((row, i) => (
        ...
        <div className="mt-20 p-12 card bg-bg-muted border-none max-w-[800px]">
          ...
          <button className="text-accent font-bold hover:underline flex items-center gap-2">
            Read the Full White Paper <ArrowRight size={18} />
          </button>
        </div>
      </div>
    </section>
...
const ROICalculator = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      savings: totalImpact.toLocaleString(undefined, { maximumFractionDigits: 0 }),
      cost: cost.toLocaleString(),
      roi: roiRatio,
      laborHrs: Math.round(employees / 50 * 1.2)
    };
          ...
          <div className="flex-1">
            <FadeIn>
              ...
              <p className="text-white/60 mb-12">
                Based on published industry research. First-party data collection begins with your pilot. Sources: Deloitte 2024, Milligan-Saville 2017.
              </p>
            </FadeIn>
            ...
            <div className="space-y-12">
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <span className="font-bold">How many employees?</span>
                  <span className="text-accent font-bold text-[20px]">{employees.toLocaleString()}</span>
                </div>
                <input
                  type="range" min="50" max="10000" step="50"
                  value={employees} onChange={(e) => {
                    const val = parseInt(e.target.value);
                    setEmployees(val);
                    updateParams(val, eapUtilization, turnoverCost);
                  }}
                  className="w-full accent-accent bg-white/10 h-1 rounded-lg appearance-none cursor-pointer"
                />
                <div className="flex justify-between text-caption text-[12px]">
                  <span>50</span>
                  <span>10,000+</span>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <span className="font-bold">Current EAP utilization rate?</span>
                  <span className="text-accent font-bold text-[20px]">{eapUtilization}%</span>
                </div>
                <input
                  type="range" min="1" max="20" step="0.5"
                  value={eapUtilization} onChange={(e) => {
                    const val = parseFloat(e.target.value);
                    setEapUtilization(val);
                    updateParams(employees, val, turnoverCost);
                  }}
                  className="w-full accent-accent bg-white/10 h-1 rounded-lg appearance-none cursor-pointer"
                />
                <div className="flex justify-between text-caption text-[12px]">
                  <span>1%</span>
                  <span>20% (national avg)</span>
                </div>
              </div>

              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <span className="font-bold">Average employee turnover cost?</span>
                  <span className="text-accent font-bold text-[20px]">${turnoverCost.toLocaleString()}</span>
                </div>
                <input
                  type="range" min="5000" max="50000" step="1000"
                  value={turnoverCost} onChange={(e) => {
                    const val = parseInt(e.target.value);
                    setTurnoverCost(val);
                    updateParams(employees, eapUtilization, val);
                  }}
                  className="w-full accent-accent bg-white/10 h-1 rounded-lg appearance-none cursor-pointer"
                />
                <div className="flex justify-between text-caption text-[12px]">
                  <span>$5K</span>
                  <span>$50K+ (industry avg)</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="card bg-white/5 border-white/10 p-10 flex flex-col items-center text-center">
                ...
                <div className="w-full space-y-4 mb-12">
                  <div className="flex justify-between text-[14px]">
                    <span className="text-white/60">Protocol deployment cost</span>
                    <span className="font-bold">$0</span>
                  </div>
                  <div className="flex justify-between text-[14px]">
                    <span className="text-white/60">Training time (10 min/manager)</span>
                    <span className="font-bold">{stats.laborHrs} hrs</span>
                  </div>
                  <div className="flex justify-between text-[14px]">
                    <span className="text-white/60">Loaded labor cost</span>
                    <span className="font-bold text-accent">${stats.cost}</span>
                  </div>
                </div>
                ...
                <div className="w-full card bg-accent p-10 border-none">
                  ...
                  <div className="pt-6 border-t border-white/20">
                    <span className="text-white/80 font-bold">ROI: {stats.roi}:1</span>
                  </div>
                </div>

                <button className="btn-primary w-full mt-10 h-14 text-lg">
      ...
      <div className="container">
        ...
        <div className="overflow-x-auto mb-16">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-hair">
                <th className="py-6 font-bold text-caption text-[12px] uppercase">Block</th>
                <th className="py-6 font-bold text-caption text-[12px] uppercase">Duration</th>
                <th className="py-6 font-bold text-caption text-[12px] uppercase">Content</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-hair">
              {blocks.map((block, i) => (
                <tr key={i}>
                  <td className="py-8 font-bold text-ink pr-8 align-top text-[18px]">{block.name}</td>
                  <td className="py-8 text-accent font-bold align-top text-[16px]">{block.time}</td>
                  <td className="py-8 text-muted align-top text-[16px]">{block.desc}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div key={i} className={cn("card p-6 flex flex-col justify-between", item.disabled && "opacity-50 grayscale")}>
              ...
              {!item.disabled && (
                <button className="text-accent text-sm font-bold mt-6 flex items-center gap-1">Download <ChevronRight size={14} /></button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 flex justify-center">
          <button className="btn-primary px-8 py-4">Download Training Kit — Free</button>
        </div>
      </div>
    </section>
  );
    ...
    <div className="container">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        {[
          ...
          <div key={i} className="card p-8 hover:shadow-md transition-all">
            <div className="w-10 h-10 rounded-full bg-bg-muted flex items-center justify-center mb-6">
              <Scale size={20} className="text-ink" />
            </div>
      ...
      <div className="card border-none bg-ink text-white p-12 mb-12">
        ...
        <ul className="grid md:grid-cols-2 gap-4">
          {[
            "A duty to diagnose or treat",
            "HIPAA obligations",
            "Clinical liability for non-licensed responders",
            "Documentation usable in performance reviews",
            "Documentation usable in termination proceedings"
          ].map(item => (
      ...
      <div className="flex flex-col md:flex-row items-center gap-12 bg-white card border-hair shadow-sm">
        <div className="flex-1">
          ...
          <p className="text-[16px] text-muted">
            "Documented response protocols strengthen your Employment Practices Liability Insurance defense posture. When an employer can demonstrate a standardized, trained response to emotional risk signals — with documented data governance controls — it materially improves their position in discrimination and accommodation claims."
          </p>
        </div>
        <button className="btn-ghost shrink-0 border-accent whitespace-nowrap px-8 h-14 font-bold flex gap-2">
          <FileText size={20} /> Download Legal One-Pager for GC
        </button>
      </div>
    </div>
  </section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {orgs.map((org, i) => (
            <div key={i} className="card flex flex-col items-start hover:bg-bg-muted transition-colors border-hair/50 p-8 h-full">
              <div className="w-12 h-12 rounded-2xl bg-bg-muted flex items-center justify-center mb-8">
                <org.icon size={24} className="text-ink" />
              </div>
              <h3 className="text-ink mb-6 font-bold text-[22px]">{org.title}</h3>
              <div className="space-y-4 mb-10 flex-grow">
                <div>
                  <div className="caption uppercase tracking-wider font-bold mb-1">Who trains</div>
                  <div className="text-ink font-medium">{org.who}</div>
                </div>
                <div>
                  <div className="caption uppercase tracking-wider font-bold mb-1">What they connect to</div>
                  <div className="text-accent font-bold">→ {org.connect}</div>
                </div>
              </div>
  ...
  <section id="sponsors" className="section bg-bg-muted/30">
    <div className="container">
      <FadeIn>
        ...
        <p className="text-[20px] mb-12 font-medium">
          [X] million people participated in Butterfly Month 2026. 12 brands were part of it.
        </p>
      </FadeIn>

      <div className="grid md:grid-cols-3 gap-6 mb-16">
        {[
          { tier: "Founding Partner", seats: "2-3 seats", features: ["Logo on every kit download + homepage", "Celebrity co-branding", "Category exclusive"] },
          { tier: "Impact Partner", seats: "4-5 seats", features: ["Logo on Resource Maps + co-branded training", "Webinar sponsorship"] },
          { tier: "Community Supporter", seats: "5-10 seats", features: ["Logo on site + social mentions", "Impact report inclusion"] }
        ].map((tier, i) => (
          <div key={i} className={cn("card p-8 flex flex-col", i === 0 ? "border-accent shadow-lg ring-1 ring-accent/20" : "border-hair shadow-sm")}>
            ...
            <button className={cn("w-full py-4 rounded-full font-bold transition-all", i === 0 ? "bg-accent text-white" : "border border-hair hover:border-accent text-accent")}>
...
const Timeline = () => (
  <section className="section bg-white">
    <div className="container">
      ...
      <div className="mt-12">
        <button className="btn-ghost border-hair hover:border-accent px-8 h-14 font-bold">
          Download 30-Day Deployment Checklist — PDF
        </button>
      </div>
    </div>
  </section>
    ...
    <div className="container">
      ...
      <div className="overflow-x-auto mb-16">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-hair">
              <th className="py-6 font-bold text-caption text-[12px] uppercase">Metric</th>
              <th className="py-6 font-bold text-caption text-[12px] uppercase">What it tells you</th>
              <th className="py-6 font-bold text-caption text-[12px] uppercase">How to collect</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-hair">
            {[
              ...
              <tr key={i}>
                <td className="py-8 font-bold text-ink pr-8 align-top text-[18px]">{row.m}</td>
                <td className="py-8 text-ink font-medium align-top text-[16px]">{row.t}</td>
                <td className="py-8 text-muted align-top text-[16px]">{row.c}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          {faqs.map((faq, i) => (
            <div key={i} className="bg-white">
              <button
                ...
                className="w-full flex items-center justify-between p-8 text-left hover:bg-bg-muted transition-colors"
              >
      ...
      <div className="container max-w-[600px]">
        ...
        >
          {[
            ...
            <div key={field.label} className="flex flex-col gap-2">
              ...
              <input
                ...
                className="bg-transparent border-b border-hair py-3 outline-none focus:border-accent transition-colors"
              />
            </div>
          ))}
          <div className="flex flex-col gap-2">
            ...
            <textarea
              rows={4}
              className="bg-transparent border-b border-hair py-3 outline-none focus:border-accent transition-colors resize-none"
            />
          </div>
  ...
  <footer className="bg-white pt-24 pb-12 border-t border-hair">
    <div className="container">
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
        ...
        <div>
          ...
          <ul className="space-y-4 caption font-medium">
            <li><a href="#" className="hover:text-ink">Protocol</a></li>
            <li><a href="#" className="hover:text-ink">Evidence</a></li>
            <li><a href="#" className="hover:text-ink">Governance</a></li>
        ...
        <div>
          ...
          <ul className="space-y-4 caption font-medium">
            ...
            <li><a href="#" className="hover:text-ink">Whistleblower</a></li>
            <li><a href="#" className="hover:text-ink font-bold text-accent">Request Investor Materials</a></li>
          </ul>
        </div>
      ...
      <div className="pt-12 border-t border-hair flex flex-col md:flex-row justify-between gap-8 text-[13px] text-caption font-medium uppercase tracking-widest">
        ...
        <div>Last updated: March 27, 2026</div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        <section className="section bg-white">
          <div className="container">
            <FadeIn>
              <span className="overline mb-4 block">VALUES AND CONSTRAINTS</span>
              <h2 className="mb-12">What governs this standard.</h2>
            </FadeIn>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { t: "Not diagnosis", d: "The protocol routes. It never diagnoses. The bright line is structural." },
                { t: "Not ideology", d: "No faith, political, or cultural framework is imposed. The script adapts." },
                { t: "Not surveillance", d: "No PII collected. No individual tracking. Aggregate data only." },
                { t: "Not optional", d: "Standards are operational, not aspirational. If deployed, it runs." }
              ].map((v, i) => (
                <div key={i} className="card p-8 bg-bg-muted border-none">
                  <h3 className="text-ink mb-4 font-bold text-[18px]">{v.t}</h3>
            ...
            <div className="flex flex-col lg:flex-row gap-16">
              <div className="flex-1">
                <FadeIn>
                  ...
                  <p className="mb-8 font-medium">One Humanity Foundation · 501(c)(3) · Published governance · Independent board</p>
                  <p className="mb-8">Developed with clinical advisors across: Psychiatry · Organizational Psychology · Emergency Medicine · Crisis Intervention</p>
                  <p className="caption">Reviewed against: OSHA · WHO mhGAP · SAMHSA · ISO 45003</p>
                </FadeIn>
              </div>
              <div className="lg:w-[400px] shrink-0">
                <div className="card border-hair bg-white grid grid-cols-1 divide-y divide-hair p-0 overflow-hidden">
                  {["White Paper", "Governance Structure", "Changelog", "Safeguarding Policy", "Whistleblower"].map(l => (
                    <a key={l} href="#" className="px-8 py-5 flex items-center justify-between hover:bg-bg-muted transition-colors font-bold text-sm">
                      {l} <ChevronRight size={16} className="text-caption" />
                    </a>
                  ))}
                </div>
              </div>
        ...
        <section className="section bg-accent-light/50 border-none py-32">
          <div className="container text-center max-w-[800px]">
            <FadeIn>
              <h2 className="mb-8">Organizations that move first define what standard looks like for their industry.</h2>
              <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                <button className="btn-primary px-10 py-5 text-lg h-auto">Download the Full Kit — Free</button>
                <button className="btn-ghost px-10 py-5 text-lg h-auto bg-white border-accent">Request Implementation Brief →</button>
              </div>
              <p className="mt-12 caption font-bold">Questions: protocol@onehumanity.org · Response within 24 business hours</p>
            </FadeIn>
          </div>
        </section>
